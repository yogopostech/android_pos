...
/// Elavon Commerce Web Services (CWS) — Base Dialog Widget
///
/// Shared UI: spinner → success/error animation → status text → log area.
/// Same look as the TSI base dialog; the log panel is enabled here.
library;
...
class CwsBaseDialog extends ConsumerStatefulWidget {
  ...
  /// Whether to show the collapsible log area (default true for CWS).
  ...
  @override
  ConsumerState<CwsBaseDialog> createState() => _CwsBaseDialogState();
}

class _CwsBaseDialogState extends ConsumerState<CwsBaseDialog> {
  ...
  void _copyLogs() {
    final logs = ref.read(cwsPaymentProvider).logs;
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(cwsPaymentProvider);
    final theme = Theme.of(context);
    const double size = 250;
    final isDone = !state.isBusy && state.phase != CwsPhase.idle;
        ...
        child: Material(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── Icon ──
              if (state.phase == CwsPhase.approved)
                SizedBox(
                  width: size,
                  height: size,
                  child: Lottie.asset(
              ...
              if (state.phase == CwsPhase.declined ||
                  state.phase == CwsPhase.error)
                SizedBox(
                  width: size,
                  height: size,
                  child: Lottie.asset(
              ...
              if (state.isBusy || state.phase == CwsPhase.idle)
                SizedBox(
                  width: size,
                  height: size,
              ...
              // ── Logs ──
              if (widget.showLogPanel && _showLogs)
                Container(
                  height: 220,
  ...
  String _phaseLabel(CwsPhase phase) => switch (phase) {
    CwsPhase.idle => 'Ready',
    CwsPhase.connecting => 'Connecting to terminal',
    CwsPhase.sending => 'Processing',
    CwsPhase.waitingForCard => 'Processing',
    CwsPhase.processing => 'Processing',
    CwsPhase.approved => 'Approved',
    CwsPhase.declined => 'Declined',
    CwsPhase.error => 'Error',
  };
}