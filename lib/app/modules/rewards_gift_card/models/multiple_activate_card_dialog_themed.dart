...
// import 'package:yogo_pos/core/constants/static_colors%20(1).dart';
// import 'package:yogo_pos/modules/rewards_gift_card/controller/gift_card_controller.dart';
// import 'package:yogo_pos/modules/rewards_gift_card/models/bulk_activate_model.dart';
// import 'package:yogo_pos/modules/rewards_gift_card/widgets/activate_all_payment_flow.dart';
// // Formatters + validators (CardNumberInputFormatter, DecimalTextInputFormatter,
// // CanadaPhoneInputFormatter, isValidCanadaPhone) activate_card_dialog theke reuse.
// import 'package:yogo_pos/modules/rewards_gift_card/widgets/card_activate_dialog.dart';
// import 'package:yogo_pos/modules/rewards_gift_card/widgets/gift_card_result_dialog.dart';
// import 'package:yogo_pos/modules/rewards_gift_card/widgets/loader.dart';
// import 'package:yogo_pos/modules/rewards_gift_card/widgets/swipe_scan_detector.dart';
...
// class _StagedCard {
//   _StagedCard({
//     required this.cardNumber, // formatted (dash soho)
//     required this.amount, // double
//     required this.name,
//     required this.phone, // formatted
//     this.status = 'staged', // staged | processing | activated | failed
//     this.error,
//   });
...
//   final GiftCardController _ctrl = Get.isRegistered<GiftCardController>()
//       ? Get.find<GiftCardController>()
//       : Get.put(GiftCardController());

//   // Palette — theme theke build() e set hoy (static const rakhle theme use kora jay na)

//   // failed card gula total + count theke baad — shudhu activate-able gula
...
//       return;
//     }
//     // OLD ta muche (cardDigits.length < 4), ei diye replace:
...
//     // ---- Duplicate card check (digits diye) ----
...
//     // ---- Server activation-check ----
//     showAppLoader();
//     final check = await _ctrl.checkActivation(card);
//     hideAppLoader();
//     if (!mounted) return;

//     // Hard failure (network / body-less) -> controller error message.
//     if (check == null) {
//       await showGiftCardResultDialog(
//         context,
//         success: false,
...
//     // status == false -> server message ta validation error hisebe.
//     if (!check.status) {
//       await showGiftCardResultDialog(
//         context,
//         success: false,
...
//     // ---- status == true -> validation thik, list e add ----
//     setState(() {
//       _cards.add(
...
//     // Focus abar card field e — pore card druto add korar jonno
//     // _cardFocus.requestFocus();
//     // _detectorKey.currentState?.resume();
//     // Add hoye gele scan-ready (notun card er jonno)
//     _detectorKey.currentState?.resume();
//   }

//   //
//   // ---- Repeat: last-added card er amount + name + phone auto-fill, card no khali ----
//   void _repeatLast() {
//     if (_activating || _cards.isEmpty) return;
//     final last = _cards.last;

//     setState(() {
//       // card number ke touch korbo na — ja type/scan kora ache thakbe
//       _amountCtrl.text = last.amount.toStringAsFixed(2);
//       _nameCtrl.text = last.name;
//       _phoneCtrl.text = last.phone;
//     });

//     // Card khali thakle card field e focus, na hole amount e
...
//   // ---- Ekta card activate (retry er jonno — single activate endpoint) ----
...
//   // ---- Sob card ek shathe bulk activate (payment flow soho) ----
//   Future<void> _activateAll() async {
//     if (_activating || _cards.isEmpty) return;

//     // Already-activated bad diye baki gula
//     final pending = _cards.where((c) => c.status != 'activated').toList();
//     if (pending.isEmpty) return;

//     // 1) Payment nao (select payment -> print/no-print OR cash & card)
//     final selection = await showActivateAllPaymentFlow(context, total: _total);
//     if (selection == null) return; // user cancel
//     if (!mounted) return;

//     // 2) Request body er cards banai
...
//           // Server card list dile match kori, na dile shob activated dhori
...
//   // Cross/close chaple — list e card thakle confirm chai.
//   // Cross/close chaple — list e card thakle exit confirm chai.
//   Future<void> _confirmClose() async {
//     if (_activating) return;

//     // List khali -> shorasori close.
//     if (_cards.isEmpty) {
...
//       Navigator.of(context).pop(); // multiple dialog close
//     }
//   }

//   // selection er prothom method theke card er source enum ber kore.
...
//   // source enum: Cash, Visa, MasterCard, Amex, Interac ...
...
//         return 'Amex';
//       case 'DEBIT CARD':
//       case 'DEBIT_CARD': // ← underscore version-o
//         return 'Interac';
//       case 'OTHERS':
//         return 'Cash';
//       default:
...
//     // Theme-based palette — ei khanei boshte hobe, karon static const e
//     // Theme.of(context) use kora jay na. _confirmClose/_cardRow o same field use kore.
//   final theme = Theme.of(context);
//     final dialogBg = theme.canvasColor;
//     final fieldBg = theme.cardColor;
...
//           onDetected: (result) {
//             _fillCard(result.cardNumber); // shudhu card
//           },
//           child: SafeArea(
//             child: Padding(
...
//                   // ---- Entry form panel (SOB field EK LINE e) ----
//                   Container(
...
//                                     ),
//                                 inputFormatters: [
//                                   // Amount, Full Name, Phone — protita _MField e first e:
...
//                                 ],
//                                 // onChanged: _onAmountChanged,   ← MUCHE DAO
//                                 focusNode: _amountFocus,
//                                 fieldBg: _fieldBg,
//                                 borderColor: _borderColor,
//                                 hintColor: _hintColor,
...
//                         // ---- Buttons: right side e Add to list + Repeat ----
//                         Row(
...
//                             ),
//                             SizedBox(width: 16),
//                             // Repeat (last card er info auto-fill)
//                             OutlinedButton.icon(
...
//                   // ---- Staging list (bacha jayga full nibe + scroll) ----
//                   Expanded(
//                     child: Container(
//                       decoration: BoxDecoration(
//                         borderRadius: BorderRadius.circular(10),
...
//         break;
//       case 'failed':
//         // failed e Retry + delete (swipe-o kora jabe)
//         trailing = Row(
//           mainAxisSize: MainAxisSize.min,
//           children: [
//             Text(
...
//           splashRadius: 20,
//           icon: const Icon(Icons.close, color: _hintColor, size: 30),
//         );
//     }
...
//     // Shudhu failed card e swipe-to-delete on. Baki gula (staged/processing/
//     // activated) e swipe off — activated card bhule delete howa theke bacha.
...
// /// Compact labeled field — multiple dialog er entry form er jonno.
// /// Size/padding gula tomar customized value.
